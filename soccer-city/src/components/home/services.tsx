...
/* Icônes Lucide associées à chaque service (Megaphone ≈ arbitre/sifflet). */
const SERVICES = [
          ...
          {SERVICES.map(({ Icon, label }) => (
            <StaggerItem key={label}>
              <div className="group flex flex-col items-center gap-3 rounded-lg glass p-6 text-center transition-all duration-300 hover:-translate-y-1.5 hover:border-primary/50 hover:shadow-glow-sm">
                ...
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground group-hover:text-foreground">{label}</p>
              </div>
            </StaggerItem>
          ))}
        </Stagger>
