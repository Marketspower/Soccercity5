...
/** Grille tarifaire — l'offre « Journée » est mise en avant. */
      ...
      <Stagger className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        {PRICING.map((plan) => (
          <StaggerItem key={plan.id} className="h-full">
            ...
            >
              {plan.highlighted && (
                <Badge className="absolute -top-3 left-6 font-display uppercase italic tracking-wider">Populaire</Badge>
              )}
              <h3 className="font-display text-sm font-bold uppercase italic tracking-widest text-muted-foreground">{plan.name}</h3>
              ...
              <Button asChild variant={plan.highlighted ? "brand" : "outline"} className="mt-8 w-full group">
                <Link href={plan.price > 200 ? "/evenements" : "/reservation"}>
                  Choisir cette offre <ArrowRight className="transition-transform group-hover:translate-x-1" />
                </Link>
              </Button>
            </article>
          </StaggerItem>
