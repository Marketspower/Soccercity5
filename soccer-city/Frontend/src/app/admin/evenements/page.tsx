  ...
  return (
    <div className="space-y-6 p-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">Événements privés</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {rows.length} demande(s) affichée(s)
          </p>
        </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-display text-lg font-bold">
                    {e.type} · {e.guests} pers.
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {e.firstName} {e.lastName}
                    {e.company ? ` — ${e.company}` : ""} · {e.date}
                  </p>
                ...
                <Badge 
                  variant={
                    e.status === "accepted" ? "pitch" : 
                    e.status === "new" ? "warning" : 
                    "destructive"
                  }
                >
              ...
              {e.status === "new" && (
                <div className="mt-4 flex gap-2">
                  <Button 
                    ...
                    size="sm" 
                    className="flex-1" 
                    onClick={() => setEventStatus(e.id, "accepted")}
                  >
                    <Check /> Accepter
                  </Button>
                  <Button 
