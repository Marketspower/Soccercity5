...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "group flex flex-1 items-center justify-between gap-4 py-5 text-left font-display text-base font-bold transition-colors hover:text-primary [&[data-state=open]]:text-primary [&[data-state=open]>svg]:rotate-45",
        className
      )}
      {...props}
    >
