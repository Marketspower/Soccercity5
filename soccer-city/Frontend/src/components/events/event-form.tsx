    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="glass rounded-lg p-7 md:p-9">
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input id="e-lastname" placeholder="Tremblay" {...register("lastName")} />
          {errors.lastName && <p className="text-xs text-destructive">{errors.lastName.message}</p>}
        </div>
        <div className="space-y-2">
          <Label htmlFor="e-firstname">Prénom</Label>
          <Input id="e-firstname" placeholder="Alex" {...register("firstName")} />
        ...
        <div className="space-y-2 sm:col-span-2">
          ...
          <Input id="e-company" placeholder="Nom de votre organisation" {...register("company")} />
        </div>
        <div className="space-y-2">
          <Label htmlFor="e-phone">Téléphone</Label>
          <Input id="e-phone" type="tel" placeholder="(450) 555-0192" {...register("phone")} />
          {errors.phone && <p className="text-xs text-destructive">{errors.phone.message}</p>}
        </div>
        <div className="space-y-2">
          <Label htmlFor="e-email">Courriel</Label>
          <Input id="e-email" type="email" placeholder="alex@exemple.ca" {...register("email")} />
          {errors.email && <p className="text-xs text-destructive">{errors.email.message}</p>}
        </div>
        <div className="space-y-2">
          <Label htmlFor="e-date">Date souhaitée</Label>
          <Input id="e-date" type="date" min={new Date().toISOString().split("T")[0]} {...register("date")} />
          {errors.date && <p className="text-xs text-destructive">{errors.date.message}</p>}
        </div>
        <div className="space-y-2">
          <Label htmlFor="e-guests">Nombre de personnes</Label>
          <Input id="e-guests" type="number" min={1} max={500} placeholder="30" {...register("guests")} />
        ...
        <div className="space-y-2 sm:col-span-2">
          ...
          <Textarea id="e-message" placeholder="Décrivez votre événement…" {...register("message")} />
